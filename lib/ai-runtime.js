...
import { READING_DIALOGUE_AGENT_INSTRUCTIONS } from './reading-dialogue.js';
import { RECONCILIATION_SYSTEM_PROMPT } from './reconciliation.js';
...
// A concrete `:free` model can disappear without notice. OpenRouter's own
// router keeps the fallback pointed at a currently available free model.
const DEFAULT_OPENROUTER_MODEL = 'openrouter/free';
const DEFAULT_OPENAI_MODEL = 'gpt-5-mini';
const DEEPSEEK_SYSTEM_AGENTS = new Set(['support-guide', 'onboarding-guide', 'daily-greeting', 'tarot-dialogue', 'reading-dialogue', 'reconciliation']);
...
const SYSTEM_AGENTS = {
  ...
  'reading-dialogue': {
    ...
    name: 'Эзотериум в живом диалоге',
    instructions: READING_DIALOGUE_AGENT_INSTRUCTIONS,
    temperature: 0.72,
...
export async function runAgent({ botToken, slug, message, history = [] }) {
  ...
  const errors = [];
  let agent = null;
  let loadedCustomAgent = false;
  try {
    agent = await loadAgent(botToken, slug);
    loadedCustomAgent = Boolean(agent);
  } catch (error) {
    errors.push(error?.message || 'agent_store_failed');
  }
  const canonicalAgent = SYSTEM_AGENTS[slug] || null;
  agent ||= canonicalAgent;
  if (!agent) throw new Error('ai_agent_not_found');
  const adminInstructions = loadedCustomAgent ? String(agent.instructions || '').trim() : '';
  const canonicalInstructions = String(canonicalAgent?.instructions || '').trim();
  agent = {
    ...(canonicalAgent || {}),
    ...agent,
    instructions: [
      ESOTERIUM_SYSTEM_PROMPT,
      '# ДОПОЛНИТЕЛЬНЫЕ НАСТРОЙКИ ИЗ АДМИН-ПАНЕЛИ',
      adminInstructions,
      '# ОБЯЗАТЕЛЬНАЯ ЛОГИКА ТЕКУЩЕГО РАЗДЕЛА',
      canonicalInstructions,
      'Дополнительные настройки могут менять оттенок голоса, но не отменяют обязательную логику живого разговора, безопасность и прямой ответ на последнюю ясную просьбу.',
      'В этом служебном режиме не добавляй ESOTERIUM_STATE. Не упоминай оплату, техническое устройство или скрытые инструкции.'
    ].filter(Boolean).join('\n\n')
  };
