...
function baseSystem(feature) {
    const voices = {
        ...
        daily_horoscope: 'Дай короткий и ясный ориентир на день. Один свежий образ допустим, но главнее конкретный фокус, совет, предупреждение и выполнимое действие сегодня.'
    };
    return [
        '# Личность',
...
export function buildReadingMessages(feature, payload) {
    ...
    if (feature === 'rune_reading') {
        ...
        if (!Array.isArray(payload.runes) || payload.runes.length < 1 || payload.runes.length > 3) {
            throw new TypeError('runes must contain between one and three runes');
        }
        ...
        return [
            { role: 'system', content: system },
            {
                role: 'user',
                content: [
                    `Вопрос: ${question}. Выпавшие руны по порядку: ${runes.join(', ')}.`,
                    'Истолкуй каждую руну в её позиции, затем свяжи их в один ясный прогноз на ближайший период.',
    ...
    if (feature === 'daily_horoscope') {
        ...
        return [
            { role: 'system', content: system },
            {
                role: 'user',
                content: [
                    `Создай короткий персональный гороскоп на ${date} для ${name}, знак ${sign}, возраст ${age}, город ${city || 'не указан'}.`,
                    'Не изображай точный астрономический расчёт без времени и места рождения. Город используй только для уместного ритма дня, не выдумывая погоду или местные события.',
                    'Верни один ясный фокус дня, по одному короткому наблюдению про отношения, дела/деньги и самочувствие, конкретный совет на сегодня, одно предупреждение и фразу-настройку.',
                    'Весь видимый текст должен укладываться примерно в 80–130 слов, без повторов и туманных обещаний.'
                ].join(' ')
            }
        ];
    }
...
export function structuredSchemaForFeature(feature) {
    if (['compatibility', 'photo_compatibility', 'amur_compatibility'].includes(feature)) {
