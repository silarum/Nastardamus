...
function baseSystem(feature, locale = 'ru') {
    const voices = {
        ...
        palm_reading: 'Начни чтение ладони с прямого ответа на вопрос человека. Опирайся только на реально видимые линии и форму; отделяй наблюдение от символического смысла. Это первая содержательная реплика живого диалога, а не итог анкеты. Не угадывай судьбу по неразличимым деталям.',
...
export function buildReadingMessages(feature, payload) {
    ...
    if (feature === 'sports_forecast') {
        const event = cleanText(payload.event, 'event', { maxLength: 500 });
                ...
                content: [
                    {
                        ...
                        text: [
                            ...
                            answers.length
                                ? `Значимые уточнения пользователя из разговора: ${answers.map((answer, index) => `${index + 1}) ${answer}`).join(' ')}`
                                : 'Предварительной анкеты не было. Ответь сразу на главный запрос и используй только доступные данные.',
                            ...
                            'Сопоставь видимые наблюдения с главным запросом, назови два подтверждающих признака, одно возможное противоречие и три конкретных шага.',
        ...
        return [
            { role: 'system', content: system },
            {
                ...
                content: [
                    ...
                    'Ресурс — 1 короткое предложение о телесном или эмоциональном ритме без медицинских утверждений.',
                    'Шаг дня — одно ясное действие, которое можно выполнить сегодня.',
                    'Вопрос Эзотериума — один короткий вопрос для личной рефлексии.',
                    'Заверши отдельной строкой «— Эзотериум». Общий объём 90–130 слов: ёмко, конкретно и без воды. Каждая строка должна добавлять новый смысл; никаких фатальных обещаний, универсальных клише, страха и повторов.'
                ].join(' ')
            }
        ];
    }
