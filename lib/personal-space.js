...
export function normalizePersonalEvent(value = {}, { allowPast = false, today = personalDateKey() } = {}) {
  ...
  return {
    ...
    links: Array.isArray(value.links) ? value.links.map((item) => cleanText(item, 500)).filter(Boolean).slice(0, 8) : cleanText(value.links, 500).split(/[,\n]/).map((item) => item.trim()).filter(Boolean).slice(0, 8),
    desiredResult: cleanText(value.desiredResult, 500),
    reflection: cleanText(value.reflection, 1000),
    category,
    priority,
