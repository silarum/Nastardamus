...
const REPAIR_PATTERN = /(?:не\s+(?:понял|поняла|понимаешь|понимает|услышал|услышала)|ты\s+(?:вообще\s+)?(?:не\s+)?понимаешь|я\s+не\s+об\s+этом|ответь\s+(?:на\s+)?вопрос|почему\s+ты\s+так\s+думаешь|с\s+чего\s+ты\s+это\s+взял(?:а)?|опять\s+(?:шаблон|не\s+то)|не\s+туда|мимо|бред|чушь|туп(?:ой|ая|о)?|идиот|бот\b|говн|дерьм|халуп)/iu;
