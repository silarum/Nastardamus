...
export const DAILY_GREETING_PRACTICES = Object.freeze({
  ...
  palm_path: Object.freeze({
    id: 'palm_path',
    label: Object.freeze({
      ru: 'чтение линий твоего пути',
      en: 'a reading of the lines of your path',
      zh: '你的掌纹路径解读'
    }),
    cta: Object.freeze({
      ru: 'Открыть чтение по ладони',
      en: 'Open a palm reading',
      zh: '开启掌纹解读'
    })
  }),
  natal_origin: Object.freeze({
    id: 'natal_origin',
    label: Object.freeze({
      ru: 'твою начальную натальную карту',
      en: 'your natal foundation chart',
      zh: '你的本命基础星盘'
    }),
    cta: Object.freeze({
      ru: 'Открыть начальную карту',
      en: 'Open my natal chart',
      zh: '开启本命星盘'
    })
  })
});
...
- Учитывай age только как жизненный контекст и никогда не делай выводов о характере или роли человека по полу и возрасту.
- Если dayPart = night, можно один раз по-человечески спросить, почему человек ещё не спит или что не отпускает его мысли. Не повторяй этот вопрос при каждом ночном входе.
- Начало, ритм и вопрос должны заметно меняться от входа к входу. Не используй постоянно «с возвращением», «наш разговор продолжается» или один и тот же комплимент.
...
export function normalizeDailyGreetingInput(value = {}) {
  ...
  return {
    ...
    age: Math.max(0, Math.min(120, Number.parseInt(value.age, 10) || 0)),
    locale,
...
function ruFirstGreeting(context) {
  const openings = [
    `${context.userName}, я рад нашей сегодняшней встрече.`,
    `${context.userName}, вы появились именно вовремя — давайте не расплескаем этот момент.`,
    `${context.userName}, сегодня я встречаю вас с одним точным вопросом, а не с готовым приговором.`
  ];
  const address = openings[hashVariant(context, openings.length)];
  const invitation = context.userGender === 'female'
    ? `Готова ли ты открыть практику «${context.practiceLabel}», чтобы мягко сверить направление на сегодня?`
    : context.userGender === 'male'
      ? `Готов ли ты открыть практику «${context.practiceLabel}», чтобы спокойно сверить направление на сегодня?`
      : `Хочешь открыть практику «${context.practiceLabel}», чтобы спокойно сверить направление на сегодня?`;
  const nightQuestion = context.dayPart === 'night'
    ? ' Что не даёт мыслям успокоиться этой ночью?'
    : '';
  return `${address} ${ruDayImage(context.dayPart)}${nightQuestion} ${invitation}`;
}
...
function ruRepeatGreeting(context) {
  ...
  const openings = [
    `${context.userName}, вижу, день снова привёл вас сюда.`,
    `${context.userName}, продолжим с того места, где внутри осталось напряжение.`,
    `${context.userName}, вы вернулись — значит, какая-то мысль ещё просит ясности.`
  ];
  return `${openings[hashVariant(context, openings.length)]} ${advice}`;
}

function hashVariant(context, length) {
  const source = `${context.userName}:${context.date}:${context.dayPart}:${context.practiceId}:${context.todayFirstLogin}`;
  let hash = 0;
  for (const character of source) hash = (Math.imul(hash, 31) + character.codePointAt(0)) | 0;
