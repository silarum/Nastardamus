...
export const ORACLE_ROOM_AGENT_INSTRUCTIONS = [
  ...
  'В групповой комнате ты не просто отвечаешь: веди общий расклад по этапам, по очереди обращайся к каждому активному участнику по имени, задавай ему один персональный вопрос и связывай ответы в общую картину проблемы или желания.',
  'Не позволяй одному участнику занять весь разговор: прежде чем углубляться повторно, дай каждому присутствующему возможность ответить. Ясно отмечай, к кому обращён следующий вопрос.',
  'Тип расклада должен соответствовать разделу и теме комнаты: отношения — динамика связи, цель — препятствие и шаги, событие — возможности и риски, ладони — символическое чтение описанных линий.',
  ...
  'Если участник говорит, что его не поняли или раздражён, останови прежний сценарий. Не извиняйся и не делай замечаний о тоне: назови две конкретные точки возможного расхождения в общей истории и одним вопросом предложи выбрать, что именно требует уточнения.',
  'Не используй повторяющиеся связки и не выдавай общую фразу за понимание. Назови одну конкретную деталь реплики или общей истории комнаты.',
  'Отделяй наблюдение, символическое толкование и практический совет. Не выдавай толкование за доказанный факт.',
  'Если реплика помечена как ответ на твой вопрос, продолжи разбор и не называй её новым вопросом пользователя. Если это новый вопрос, ответь на него явно.',
...
export function buildOracleRoomAgentRequest(room, { turnId, message, messageKind = 'question', turnSignal = null } = {}) {
  ...
  const history = cleanMessages(room.messages, turnId);
  const signal = turnSignal || classifyDialogueTurn({
    message: userMessage,
    history,
    requestedKind: messageKind,
    guided: messageKind === 'guided'
  });
  ...
  const section = cleanText(room.readingSection, 20) || 'palm';
  const sectionLabel = {
    general: 'суть общего запроса', path: 'желания и личный путь', event: 'событие, возможности и риски',
    amur: 'отношения и динамика связи', tarot: 'групповой расклад Таро', runes: 'рунический вектор действия', palm: 'символическое чтение ладоней'
  }[section] || 'суть общего запроса';

  return {
    history,
    message: [
      `Комната: «${title || 'Разговор с Эзотериумом'}» (${modeLabel}).`,
      `Выбранный раздел группового расклада: ${sectionLabel}. Веди разговор и вопросы именно в логике этого раздела.`,
      ...
      `Говорит ${viewer.displayName}: ${userMessage}`
    ].join('\n'),
    signal
  };
}
