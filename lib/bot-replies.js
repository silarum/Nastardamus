...
export function buildBotReply(update, webAppUrl, { adminIds = [] } = {}) {
    ...
    if (/^\/start(?:@[A-Za-z0-9_]+)?(?:\s|$)/.test(text)) {
        ...
        const rows = [[{
            text: inviteMatch ? '🔮 Принять приглашение' : '🔮 Войти в Nastardamus',
        ...
        return {
            ...
            payload: {
                ...
                text: inviteMatch
                    ...
                    : 'Добро пожаловать в Nastardamus. Я — ваш мистический проводник по миру знаков. Войдите в приложение кнопкой ниже или спросите меня о его возможностях.',
    ...
    if (text.startsWith('/')) {
        return {
            ...
            payload: {
                chat_id: message.chat.id,
                text: 'Эта команда не относится к открытому пути Nastardamus. Используйте /start, /support или /id.'
            }
        };
    }
