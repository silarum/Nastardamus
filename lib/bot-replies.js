...
export function buildBotReply(update, webAppUrl, { adminIds = [] } = {}) {
    ...
    if (/^\/start(?:@[A-Za-z0-9_]+)?(?:\s|$)/.test(text)) {
        ...
        const reconciliationMatch = payload.match(/^reconcile_([a-f0-9]{32})$/);
        const appUrl = new URL(webAppUrl);
        if (reconciliationMatch) {
            appUrl.searchParams.set('screen', 'reconciliation-room');
            appUrl.searchParams.set('reconciliation', reconciliationMatch[1]);
        } else if (oracleRoomMatch) {
        ...
        const hasInvitation = Boolean(reconciliationMatch || oracleRoomMatch || jointInviteMatch || inviteMatch);
        const rows = [[{
        ...
        return {
            ...
            payload: {
                ...
                text: hasInvitation
                    ? reconciliationMatch
                        ? 'Вас пригласили в комнату примирения Эзотериума. Участие добровольно: сначала вы увидите цель разговора и сами решите, готовы ли войти.'
                        : oracleRoomMatch
