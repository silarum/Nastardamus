...
import { handleAdminUsersRequest } from '../lib/admin-users.js';
import { DIALOGUE_DEFAULT_CATALOG, DIALOGUE_SECTION_TITLES } from '../lib/dialogue-policy.js';
...
const SERVICE_DEFINITIONS = Object.freeze({
  ...
  sports_personal: 'Персональный спортивный разбор',
  reconciliation_create: 'Создание комнаты примирения',
  reconciliation_participation: 'Участие в примирении',
  reconciliation_group: 'Групповое примирение',
  reconciliation_runes: 'Руны в примирении',
  reconciliation_tarot: 'Таро в примирении',
  reconciliation_palmistry: 'Ладони в примирении',
  reconciliation_astrology: 'Астрология в примирении',
  reconciliation_combined: 'Комбинированный анализ примирения',
  reconciliation_outcome_card: 'Открытка об итоге примирения'
});

const DIALOGUE_DEFINITIONS = DIALOGUE_SECTION_TITLES;

const DEFAULT_DIALOGUE_CATALOG = Object.freeze(Object.fromEntries(
  Object.entries(DIALOGUE_DEFINITIONS).map(([id, title]) => [id, {
    ...DIALOGUE_DEFAULT_CATALOG[id],
    id,
    title
  }])
));
...
const DEFAULT_SERVICE_CATALOG = Object.freeze(
  Object.fromEntries(Object.entries(SERVICE_DEFINITIONS).map(([id, title]) => [
    id,
    { id, title, enabled: true, price: id === 'sports_personal' ? 10 : null }
  ]))
);
...
function sanitizeDialogueCatalog(input) {
  ...
  return Object.fromEntries(Object.entries(DIALOGUE_DEFINITIONS).map(([id, title]) => {
    ...
    return [id, {
      ...
      includedQuestions: Math.round(clampNumber(
        item.includedQuestions,
        0,
        1000,
        DIALOGUE_DEFAULT_CATALOG[id]?.includedQuestions ?? 3
      )),
      extraQuestionPrice: clampNumber(
        item.extraQuestionPrice,
        0,
        1_000_000,
        DIALOGUE_DEFAULT_CATALOG[id]?.extraQuestionPrice ?? 0.1
      )
    }];
  }));
}
...
function sanitizeReconciliation(input) {
  const source = input && typeof input === 'object' && !Array.isArray(input) ? input : {};
  ...
  const allowedTypes = new Set(['romantic', 'friendship', 'family', 'business', 'collective', 'other']);
  const conflictTypes = Array.isArray(source.conflictTypes)
    ? [...new Set(source.conflictTypes.map(String).filter((type) => allowedTypes.has(type)))].slice(0, 6)
    : DEFAULT_SETTINGS.reconciliation.conflictTypes;
  return {
    enabled: source.enabled !== false,
    invitationHours: Math.round(clampNumber(source.invitationHours, 1, 720, 72)),
    maxParticipants: Math.round(clampNumber(source.maxParticipants, 3, 10, 10)),
    prices: {
      create: clampNumber(prices.create, 0, 1_000_000, 10),
      participate: clampNumber(prices.participate, 0, 1_000_000, 5),
      group: clampNumber(prices.group, 0, 1_000_000, 30),
      runes: clampNumber(prices.runes, 0, 1_000_000, 10),
      tarot: clampNumber(prices.tarot, 0, 1_000_000, 10),
      palmistry: clampNumber(prices.palmistry, 0, 1_000_000, 15),
      astrology: clampNumber(prices.astrology, 0, 1_000_000, 15),
      combined: clampNumber(prices.combined, 0, 1_000_000, 25),
      outcomeCard: clampNumber(prices.outcomeCard, 0, 1_000_000, 5)
    },
    tools: {
      runes: tools.runes !== false,
      tarot: tools.tarot !== false,
      palmistry: tools.palmistry !== false,
      astrology: tools.astrology !== false,
      combined: tools.combined !== false
    },
    conflictTypes: conflictTypes.length ? conflictTypes : ['other'],
    invitationText: cleanText(source.invitationText, 700) || DEFAULT_SETTINGS.reconciliation.invitationText,
    outcomeText: cleanText(source.outcomeText, 400) || DEFAULT_SETTINGS.reconciliation.outcomeText
  };
}
...
function sanitizeSettings(input = {}) {
  ...
  return {
    ...
    reconciliation: sanitizeReconciliation(input.reconciliation)
  };
}
...
async function directStorageRequest(path, options = {}) {
  const config = getSupabaseConfig();
  if (!config) return null;
...
function normalizeQuestAnswer(value) {
  return cleanText(value, 180).replace(/\s+/g, ' ').toLocaleLowerCase('ru');
}

function campaignPoster(value) {
  const source = String(value || '');
  const match = source.match(/^data:(image\/(?:jpeg|png|webp));base64,([A-Za-z0-9+/=\s]+)$/);
  if (!match) return null;
  const bytes = Buffer.from(match[2].replace(/\s+/g, ''), 'base64');
  if (bytes.length < 64 || bytes.length > 2 * 1024 * 1024) return null;
  return { contentType: match[1], bytes };
}

async function uploadCampaignPoster(campaignId, value) {
  const image = campaignPoster(value);
  if (!image) return '';
  const extension = image.contentType === 'image/png' ? 'png' : image.contentType === 'image/jpeg' ? 'jpg' : 'webp';
  const path = `${campaignId}/poster.${extension}`;
  await directStorageRequest(`object/nastardamus-campaign-posters/${path}`, {
    method: 'POST',
    headers: { 'Content-Type': image.contentType, 'x-upsert': 'true', 'Cache-Control': 'public,max-age=3600' },
    body: image.bytes
  });
  const config = getSupabaseConfig();
  return `${config.url}/storage/v1/object/public/nastardamus-campaign-posters/${path}`;
}

async function listAdminCampaigns() {
  const response = await directSupabaseRequest('nastardamus_campaigns?select=id,kind,title,description,action_url,poster_url,status,total_slots,remaining_slots,reward_units,prize_units,winner_telegram_id,starts_at,ends_at,created_at,updated_at&order=created_at.desc');
  return response ? response.json() : [];
}

async function saveAdminCampaign(input, adminId) {
  const id = /^[0-9a-f-]{36}$/i.test(String(input?.id || '')) ? String(input.id) : randomUUID();
  const kind = input?.kind === 'quest' ? 'quest' : 'task';
  const title = cleanText(input?.title, 120);
  const description = cleanText(input?.description, 1200);
  if (title.length < 3 || description.length < 3) throw new Error('invalid_campaign');
  const totalSlots = Math.round(clampNumber(input?.totalSlots, 1, 1_000_000, 1));
  const remainingSlots = Math.round(clampNumber(input?.remainingSlots, 0, totalSlots, totalSlots));
  const answer = normalizeQuestAnswer(input?.answer);
  if (kind === 'quest' && !answer && !input?.id) throw new Error('quest_answer_required');
  const posterUrl = await uploadCampaignPoster(id, input?.posterDataUrl)
    || cleanHttpsUrl(input?.posterUrl)
    || null;
  const payload = {
    id,
    kind,
    title,
    description,
    action_url: cleanHttpsUrl(input?.actionUrl) || null,
    poster_url: posterUrl,
    status: ['draft', 'active', 'paused', 'completed', 'archived'].includes(input?.status) ? input.status : 'draft',
    total_slots: totalSlots,
    remaining_slots: remainingSlots,
    reward_units: kind === 'task' ? Math.round(clampNumber(input?.reward, 0, 1_000_000, 0) * 100) : 0,
    prize_units: kind === 'quest' ? Math.round(clampNumber(input?.prize, 0, 1_000_000, 0) * 100) : 0,
    ...(answer ? { answer_hash: createHash('sha256').update(answer).digest('hex') } : {}),
    starts_at: input?.startsAt ? new Date(input.startsAt).toISOString() : null,
    ends_at: input?.endsAt ? new Date(input.endsAt).toISOString() : null,
    created_by: adminId,
    updated_at: new Date().toISOString()
  };
  const response = await directSupabaseRequest(`nastardamus_campaigns?on_conflict=id`, {
    method: 'POST',
...
function enhanceControlFile(control, source) {
  ...
  if (control === 'page') {
    body = body.replace(
      /<nav class="admin-tabs admin-sidebar"[\s\S]*?<\/nav>/u,
      `<nav class="admin-tabs admin-sidebar" aria-label="Разделы админ-панели">
        <div class="sidebar-caption"><span>РАБОЧИЕ БЛОКИ</span></div>
        <details class="admin-nav-group" open><summary>Главное</summary>
          <button type="button" class="active" data-tab="overview"><b>◈</b><span>Обзор</span></button>
          <button type="button" data-tab="users"><b>♙</b><span>Пользователи</span></button>
        </details>
        <details class="admin-nav-group" open><summary>Услуги и финансы</summary>
          <button type="button" data-tab="content"><b>✦</b><span>Контент</span></button>
          <button type="button" data-tab="dialogues"><b>⌁</b><span>Лимиты сообщений</span></button>
          <button type="button" data-tab="payments"><b>◇</b><span>Платежи и цены</span></button>
          <button type="button" data-tab="popularity"><b>↗</b><span>Использование</span></button>
        </details>
        <details class="admin-nav-group" open><summary>Активности</summary>
          <button type="button" data-tab="campaigns"><b>⚑</b><span>Задания и квесты</span></button>
          <button type="button" data-tab="growth"><b>↗</b><span>Рост и подарки</span></button>
        </details>
        <details class="admin-nav-group"><summary>Управление</summary>
          <button type="button" data-tab="support"><b>◌</b><span>Поддержка</span></button>
          <button type="button" data-tab="team"><b>♜</b><span>Команда</span></button>
          <button type="button" data-tab="ai"><b>⌁</b><span>Центр ответов</span></button>
          <button type="button" data-tab="audit"><b>≋</b><span>Журнал</span></button>
    ...
    body = body.replace(
      '            </div>\n          </section>\n\n          <section class="card panel" data-settings-domain="growth">',
      '              <div class="service-price-row" data-service="sports_personal"><label class="switch-row"><span><strong>Персональный спортивный разбор</strong></span><input data-service-enabled type="checkbox" checked></label><label>Цена, SILARUM<input data-service-price type="number" min="0" step="0.01" value="10"></label></div>\n            </div>\n          </section>\n\n          <section class="card panel" data-settings-domain="growth">'
    );
    body = body.replace(
      '<section class="tab-panel" data-panel="payments" hidden>',
    ...
    body = body.replace(
      '<section class="tab-panel" data-panel="payments" hidden>',
      '<section class="tab-panel" data-panel="campaigns" hidden>\n'
        + '  <div class="section-heading"><div><p class="eyebrow">Активности</p><h2>Задания и квесты</h2><p>Создавайте ограниченные задания с наградой или квест, где приз получает первый правильный ответ.</p></div><button type="button" class="mini-button" id="refresh-campaigns">Обновить</button></div>\n'
        + '  <section class="card panel campaign-admin-guide"><strong>Как пользоваться</strong><p>Выберите тип, добавьте понятное описание и ссылку. Для задания задайте награду каждому участнику. Для квеста укажите секретный правильный ответ и общий приз. «Количество мест» уменьшается после каждой принятой отправки.</p></section>\n'
        + '  <form id="campaign-form" class="card panel campaign-admin-form">\n'
        + '    <input name="id" type="hidden"><div class="two-cols"><label>Тип<select name="kind"><option value="task">Задание</option><option value="quest">Квест</option></select></label><label>Состояние<select name="status"><option value="draft">Черновик</option><option value="active">Опубликовано</option><option value="paused">Приостановлено</option><option value="completed">Завершено</option></select></label></div>\n'
        + '    <label>Название<input name="title" maxlength="120" required placeholder="Например: Подпишитесь на канал"></label><label>Описание<textarea name="description" rows="4" maxlength="1200" required placeholder="Что сделать, где проверить и когда начислится награда"></textarea></label>\n'
        + '    <label>Ссылка выполнения, HTTPS<input name="actionUrl" type="url" maxlength="1000" placeholder="https://..."></label><div class="two-cols"><label>Всего мест<input name="totalSlots" type="number" min="1" max="1000000" value="1000"></label><label>Осталось мест<input name="remainingSlots" type="number" min="0" max="1000000" value="1000"></label></div>\n'
        + '    <div class="two-cols"><label data-campaign-task>Награда каждому, SILARUM<input name="reward" type="number" min="0" step="0.01" value="1"></label><label data-campaign-quest hidden>Приз победителю, SILARUM<input name="prize" type="number" min="0" step="0.01" value="10"></label></div>\n'
        + '    <label data-campaign-quest hidden>Правильный ответ<input name="answer" maxlength="180" autocomplete="off" placeholder="Хранится как защищённый хэш"></label>\n'
        + '    <label>Афиша (необязательно)<input name="poster" type="file" accept="image/jpeg,image/png,image/webp"></label><img id="campaign-poster-preview" class="campaign-poster-preview" alt="" hidden>\n'
        + '    <div class="drawer-actions"><button type="submit">Сохранить</button><button type="button" class="secondary-button" id="reset-campaign-form">Новая запись</button></div>\n'
        + '  </form><section class="card panel"><div class="panel-head"><div><p class="eyebrow">Опубликованные и черновики</p><h2>Список активностей</h2></div></div><div id="campaign-admin-list" class="campaign-admin-list"><p class="empty-state">Загрузка…</p></div></section>\n'
        + '</section>\n\n<section class="tab-panel" data-panel="payments" hidden>'
    );
...
export default async function handler(req, res) {
  ...
  if (req.query?.adminUsers) {
    return handleAdminUsersRequest(req, res);
  }
  ...
  try {
    if (req.method === 'GET') {
      if (req.query?.campaigns === '1') {
        if (!hasPermission(profile, 'settings.manage')) {
          return sendJson(res, 403, { error: 'permission_denied' });
        }
        return sendJson(res, 200, { ok: true, campaigns: await listAdminCampaigns() });
      }
    ...
    if (req.body?.campaignAction === 'save') {
      if (!hasPermission(profile, 'settings.manage')) {
        return sendJson(res, 403, { error: 'permission_denied' });
      }
      const campaign = await saveAdminCampaign(req.body?.campaign, userId);
      await writeAudit(userId, 'campaign_saved', { campaignId: campaign?.id, kind: campaign?.kind, status: campaign?.status }, botToken);
      return sendJson(res, 200, { ok: true, campaign });
    }

    if (req.body?.campaignAction === 'archive') {
      if (!hasPermission(profile, 'settings.manage')) {
        return sendJson(res, 403, { error: 'permission_denied' });
      }
      const campaignId = String(req.body?.campaignId || '');
      if (!/^[0-9a-f-]{36}$/i.test(campaignId)) return sendJson(res, 400, { error: 'invalid_campaign_id' });
      await directSupabaseRequest(`nastardamus_campaigns?id=eq.${encodeURIComponent(campaignId)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Prefer: 'return=minimal' },
        body: JSON.stringify({ status: 'archived', updated_at: new Date().toISOString() })
      });
      await writeAudit(userId, 'campaign_archived', { campaignId }, botToken);
      return sendJson(res, 200, { ok: true });
    }
