...
function parseAdminIds(value) {
  return new Set(String(value || '').split(/[\s,;]+/).map(Number).filter(Number.isSafeInteger));
}
...
async function readSettings() {
  ...
  if (!url || !key) return { settings: DEFAULT_SETTINGS, persisted: false };
  const response = await fetch(`${url}/rest/v1/nastardamus_settings?key=eq.global&select=settings&limit=1`, {
...
async function writeSettings(settings) {
  ...
  const response = await fetch(`${url}/rest/v1/nastardamus_settings?on_conflict=key`, {
    method: 'POST',
    headers: {
      apikey: key,
  ...
  if (!response.ok) throw new Error(`settings_write_${response.status}`);
  return true;
}
...
export default async function handler(req, res) {
  ...
  if (!validation.ok) return sendJson(res, 401, { error: 'telegram_auth_required', reason: validation.reason });

  const userId = Number(validation.user.id);
  const adminIds = parseAdminIds(process.env.ADMIN_TELEGRAM_IDS);
  if (adminIds.size === 0) return sendJson(res, 503, { error: 'admin_ids_not_configured', userId });
  if (!adminIds.has(userId)) return sendJson(res, 403, { error: 'admin_access_denied', userId });
  ...
  try {
    if (req.method === 'GET') {
      ...
      return sendJson(res, 200, {
        ...
        services: {
          bot: Boolean(process.env.BOT_TOKEN || process.env.ADMIN_BOT_TOKEN),
