...
export default async function handler(req, res) {
  ...
  if (!validation.ok && !previewAllowed) {
    return sendJson(res, 401, { error: 'telegram_auth_required' });
  }

  const userId = validation.ok ? Number(validation.user.id) : 1;
  ...
  try {
    ...
    if (['withdrawals_disabled', 'below_minimum', 'insufficient_funds', 'invalid_destination'].includes(code)) {
